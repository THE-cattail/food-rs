...
impl<'de, T> Parser<'de, T> where T: Deserialize<'de>
{
...
pub fn parse<'de, T>(buf: &'de [u8]) -> Result<T>
    where T: Deserialize<'de>
{
    let parsers: Vec<Parser<'de, T>> =
        vec![Parser::new("toml", parser_fn!(toml::from_slice)),
    ...
    Err(anyhow::anyhow!("unknown config file format, only \
                         support: [{}]",
                        parsers.into_iter()
